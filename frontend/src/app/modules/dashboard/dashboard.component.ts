...
@Component({
    ...
    <div class="content">
      ...
      <div class="grid">
        @if (authService.hasAnyRole('ADMIN', 'MANAGER')) {
          <mat-card class="card" routerLink="/users">
            <mat-icon class="card-icon">manage_accounts</mat-icon>
  ...
  styles: [`
    .spacer { flex: 1; }
    .user-info { font-size: 14px; margin-right: 12px; opacity: 0.9; }
    .content { padding: 24px; max-width: 1200px; margin: 0 auto; }
    .grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(260px, 1fr)); gap: 16px; margin-top: 16px; }
    .card { cursor: pointer; padding: 24px; transition: box-shadow 0.2s; }
